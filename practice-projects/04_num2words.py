...
def normal_convert():
    while True:
        number = eval(input('Enter a number, will give you in Word Format: '))
        if (isinstance(number, str) and number.isdigit()) or isinstance(number, int):
            number = int(number)
            num_wrds = str(n2w.num2words(number=number))
            print(number, num_wrds.title())
            break
        else:
            print('Please check the input!!!')


# using tkinter, showing in GUI
def num_to_words():
...
photo = PhotoImage(file = "../util-libs/number.png")
